      ...
      return {
        ...
        run: async function(params = []) {
          ...
          trOdds.each(function() {
            console.log($(this).not('[colspan=2]'));
          ...
          const fieldNames = Custom.getFieldNames(widget, 'form_2', 'form_3');
...
Словосочетание "Темп. режим" подставляется если в поле "Темп. режим." стоит флаг, 0 +2 подставляется из поля "Коммент. к режиму"
          ...
          * */
          ...
          const cargoCostCurrency = params.common['currency'].value;
          const cargoCostRow = that.renderSpan(['Стоимость груза', `${cargoCost} ${cargoCostCurrency}`]);
          /*
      ...
      "mode_comment": "Коммент. к режиму",
        ...
        generateCostTable: function(params) {
          ...
          if (arrivalHtml !== '') {
            arrivalHtml = `${that.renderColSpan('до пункта прилета')}${arrivalHtml}${that.renderTd(
                ['ВСЕГО ДО ПУНКТА ПРИЛЕТА', arrival.total, arrival.currency], 'font-weight: bold;')}`;
          }
          ...
          if (table) {
            table.rows.forEach(row => {
              ...
              if (row.value !== undefined) {
                ...
                if (row.code === 'air_fare' || row.code === 'awb') {
                  styles = 'font-weight: bold; border: 2px solid black;';
                }

                const formattedValue = Custom.formatFloat(row.value);
                html += that.renderTd([row.title, formattedValue, currency + dimension], styles);
              }
        ...
        planeImg: function() {
          return '<img style=" height: 50px; margin: 0 17px; "' +
              ' src="https://scs-aero.ru/wp-content/uploads/2022/01/airplane.png"/>';
          //' src="https://dev.n1k.cc/assets/plane.png"/>';

        },

        carImg: function(style) {
          return `<img style="height: 50px; ${style}"  src="https://scs-aero.ru/wp-content/uploads/2022/01/truck.png"/>`;
        },
